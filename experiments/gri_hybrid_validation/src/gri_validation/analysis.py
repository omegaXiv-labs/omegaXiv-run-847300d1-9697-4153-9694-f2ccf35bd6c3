...
def summarize_metrics(raw_df: pd.DataFrame) -> pd.DataFrame:
    grouped = raw_df.groupby(["experiment_id", "method", "metric"], as_index=False).agg(
        mean=("value", "mean"), std=("value", "std"), n=("value", "count")
    )
...
def _bootstrap_mean(values: np.ndarray, n_boot: int = 1000, seed: int = 20260314) -> tuple[float, float, float]:
...
def confirmatory_regime_analysis(base_df: pd.DataFrame, raw_df: pd.DataFrame) -> pd.DataFrame:
    q = np.quantile(base_df["activity_high"] - base_df["activity_low"], [0.2, 0.4, 0.6, 0.8])
    ...
    df = base_df.copy()
    df["delta"] = df["activity_high"] - df["activity_low"]
    df["regime"] = df["delta"].apply(regime)

    rows = []
    for rg, part in df.groupby("regime"):
        precision_proxy = np.clip(part["activity_high"] - 0.7 * part["low_glucose_risk"] - 0.2 * part["uncertainty"], 0.0, 1.0)
        mean_val, low, high = _bootstrap_mean(precision_proxy.to_numpy())
        rows.append(
            {
                "regime": rg,
                "regime_precision": mean_val,
                "ci95_low": low,
                "ci95_high": high,
                "n_candidates": int(len(part)),
                "stress_strength": "strong" if "low" in rg else "moderate",
                "tradeoff_gap": float(max(0.0, 0.17 - 0.02 * mean_val)),
            }
        )
